...
class DisNet_pico_deep(nn.Module):
    def __init__(self, out_channels,  config_dict=None):
        super(DisNet_pico_deep, self).__init__()

        self.config_dict = config_dict
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=config_dict['dim_1'], kernel_size=config_dict['kernel_1'], padding='same')
        nn.init.kaiming_normal_(self.conv1.weight, mode='fan_out', nonlinearity='relu')
        self.cnblock1 = CNBlock_pico(dim=config_dict['dim_1'], kernel_3=config_dict['kernel_3'], kernel_4=config_dict['kernel_4'])
        ...
        nn.init.kaiming_normal_(self.conv2.weight, mode='fan_out', nonlinearity='relu')
        self.cnblock2 = CNBlock_pico(dim=config_dict['dim_2'], kernel_3=config_dict['kernel_5'], kernel_4=config_dict['kernel_6'])

        self.pool = nn.AvgPool2d(2, 2)

        self.conv3 = nn.Conv2d(in_channels=config_dict['dim_2'], out_channels=config_dict['dim_3'], kernel_size=config_dict['kernel_7'], padding='same')
        nn.init.kaiming_normal_(self.conv3.weight, mode='fan_out', nonlinearity='relu')
        self.cnblock3 = CNBlock_pico(dim=config_dict['dim_3'], kernel_3=config_dict['kernel_8'], kernel_4=config_dict['kernel_9'])

        self.conv4 = nn.Conv2d(in_channels=config_dict['dim_3'], out_channels=config_dict['dim_4'], kernel_size=config_dict['kernel_10'], padding='same')
        nn.init.kaiming_normal_(self.conv4.weight, mode='fan_out', nonlinearity='relu')
        self.cnblock4 = CNBlock_pico(dim=config_dict['dim_4'], kernel_3=config_dict['kernel_11'], kernel_4=config_dict['kernel_12'])

        self.fc1 = nn.Linear(160000, config_dict['nodes_1'])
        nn.init.kaiming_normal_(self.fc1.weight, mode='fan_out', nonlinearity='relu')
...
class DisNet_pico(nn.Module):
    def __init__(self, out_channels, config_dict):
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=config_dict['dim_1'], kernel_size=config_dict['kernel_1'], padding='same') 
        nn.init.kaiming_normal_(self.conv1.weight, mode='fan_out', nonlinearity='relu')
        self.cnblock1 = CNBlock_pico(dim=config_dict['dim_1'], kernel_3=config_dict['kernel_3'], kernel_4=config_dict['kernel_4'], dropout=config_dict['dropout'])
        self.pool = nn.AvgPool2d(2, 2)
        self.conv2 = nn.Conv2d(in_channels=config_dict['dim_1'], out_channels=config_dict['dim_2'], kernel_size=config_dict['kernel_2'], padding='same')
        nn.init.kaiming_normal_(self.conv2.weight, mode='fan_out', nonlinearity='relu')
        self.cnblock2 = CNBlock_pico(dim=config_dict['dim_2'], kernel_3=config_dict['kernel_5'], kernel_4=config_dict['kernel_6'], dropout=config_dict['dropout'])
