...
def train(config):
    ...
    criterion_dict = {'val': nn.CrossEntropyLoss(), 'train': DynamicFocalLoss(alpha=config['alpha'], gamma=config['gamma'])}
    ...
    optimizer = torch.optim.AdamW(model_ft.parameters(), lr=config['lr'],
                                            weight_decay=config['weight_decay'], eps=args.eps)
...
def main():
    Working_dir = "/jmain02/home/J2AD016/jjw02/jjs00-jjw02/dat/models/HypSweep"
    #empty pandas df, columns = ['Run id', 'loss', 'f1', 'lr', 'weight_decay','alpha', 'gamma', 'input_size']
    alpha_lst, gamma_lst = [1, 1.2, 1.4, 1.6, 1.8], [1, 1.2, 1.4, 1.6, 1.8]

    
    config = {'alpha': random.choice(alpha_lst), 'gamma': random.choice(gamma_lst), 'lr': sample_from_log_distribution(1e-6, 1e-3)[0],
            'weight_decay':sample_from_log_distribution(1e-4, 1e-1)[0], 'input_size':random.randint(300, 340)}

    _, best_f1, best_f1_loss = train(config)
    result_list = [best_f1_loss, best_f1, config['lr'], config['weight_decay'], config['alpha'], config['gamma'], config['input_size']]
    #append results_list to csv file
    with open(os.path.join(Working_dir, 'results.csv'), 'a') as f:
        writer = csv.writer(f)
