...
def train():
    ...
    with open("/local/scratch/jrs596/dat/budgetful_61.json") as f:
        config_dict = yaml.load(f, Loader=yaml.FullLoader)


    model_config = {'num_classes': num_classes, 'input_size': args.input_size,
                'stochastic_depth_prob': config_dict['stochastic_depth_prob'], 'layer_scale': config_dict['layer_scale'],
                'dim_1': config_dict['dim_1'], 'dim_2': config_dict['dim_2'], 
                'nodes_1': config_dict['nodes_1'], 'nodes_2': config_dict['nodes_1'],
                'kernel_1': config_dict['kernel_1'], 'kernel_2': config_dict['kernel_2'],
                'kernel_3': config_dict['kernel_3'], 'kernel_4': config_dict['kernel_4'],
                'kernel_5': config_dict['kernel_5'], 'kernel_6': config_dict['kernel_6']
      }
    ...
    dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=args.batch_size, shuffle=True, num_workers=12, worker_init_fn=toolbox.worker_init_fn, drop_last=False) for x in ['train', 'val']}
    
    #criterion = {'val': nn.CrossEntropyLoss(), 'train': toolbox.DynamicFocalLoss(delta=args.delta, dataloader=dataloaders_dict['train'])}
    criterion = nn.CrossEntropyLoss()
    
    trained_model, best_f1, best_f1_loss, best_train_f1, run_name = train_model(args=args, l1_lambda=args.l1_lambda, model=model, optimizer=optimizer, device=device, dataloaders_dict=dataloaders_dict, criterion=criterion, patience=args.patience, initial_bias=initial_bias, input_size=None, n_tokens=args.n_tokens, batch_size=args.batch_size, AttNet=None, ANoptimizer=None)
...
if args.sweep == True:
    for i in range(args.n_sweeps):
        trained_model, best_f1, best_f1_loss, best_train_f1, config = train()

        config['loss'] = best_f1_loss
        config['f1'] = best_f1
        config['train_f1'] = best_train_f1
        #make directory if it doesn't exist
        os.makedirs(os.path.join(args.root, "dat/HypSweep/", wandb.project.name), exist_ok=True)
        out_file = os.path.join(args.root, "dat/HypSweep/", wandb.project.name, config['Run_name'] + ".json")
