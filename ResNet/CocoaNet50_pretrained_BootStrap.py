...
data_dir = "/local/scratch/jrs596/dat/compiled_cocoa_images/CrossVal_split/train_val"  #even_split"
...
writer = SummaryWriter(log_dir='/local/scratch/jrs596/CocoaNet50_ImageNet/BootStrap_logs')

def randomise_data(temp_dir):
...
def train_model(model, criterion, optimizer, num_epochs=num_epochs, is_inception=False):
    since = time.time()
...
#    best_model_wts = copy.deepcopy(pretrained_model_wts['model'])
    ...
    dataloaders = randomise_data(temp_dir)
...
#            print('\nLoss to beat: ' + str(round(loss_to_beat* 0.8, 2)) + '\n')
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_loss < best_loss:
                ...
                model_path = '/local/scratch/jrs596/CocoaNet50_ImageNet/models/CocoaNet50_ImageNet_BootStrap_model.pkl'
                ...
                torch.save(model, '/local/scratch/jrs596/CocoaNet50_ImageNet/models/CocoaNet50_ImageNet_BootStrap_model.pth')
...
model, hist = train_model(model_ft, criterion, optimizer_ft, num_epochs=num_epochs, is_inception=False)
