...
def train():
    ...
    block_setting = [
        CNBlockConfig(wandb.config.one, wandb.config.two, wandb.config.k_1),
        CNBlockConfig(wandb.config.two, wandb.config.three, wandb.config.k_2),
        CNBlockConfig(wandb.config.three, wandb.config.four, wandb.config.k_3),
        CNBlockConfig(wandb.config.four, None, wandb.config.k_4)]

    # block_setting = [
    #     CNBlockConfig(90, 100, 6),
    #     CNBlockConfig(100, 155, 3),
    #     CNBlockConfig(155, 755, 8),
    #     CNBlockConfig(755, None, 3)]
    
    model = ConvNeXt(block_setting=block_setting, stochastic_depth_prob=wandb.config.stochastic_depth_prob, layer_scale=1e-6, num_classes=num_classes)
